...
public class NfaSimulator {
    ...
    public boolean simulate(NFA nfa, String input) {
        ...
        */
        ...
        addEpsilonClosure(nfa.startState, estados);
        
        for (char c : input.toCharArray()) {
            Set<State> estadosSig = new HashSet<>();
            
            for (State estado : estados) {
                for (State nextState : estado.getTransitions(c)) {
                    addEpsilonClosure(nextState, estadosSig);
                }
            }
            estados = estadosSig;
        }     
